...
export default function WorkflowInputPanel({
  ...
  const [activeGuideTab, setActiveGuideTab] = useState<"android_chrome" | "ios_safari" | "ios_chrome" | "desktop_chrome" | "mac_safari" | "desktop_firefox">("android_chrome");
  ...
  const handleTabChange = (tab: "text" | "file" | "image" | "audio") => {
    ...
    setAudioError(null);
    setMicPermissionUiState("idle");
    setActiveGuideTab("android_chrome");
    if (isRecording) {
      stopRecording();
    }
    // 인풋 리셋
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  ...
                  <div className="ux_audio_permission_notice">
                    <h5 className="ux_audio_permission_notice_title">
                      ⚠️ {(() => {
                        if (audioError === "NotAllowedError") {
                          return micPermissionUiState === "blocked"
                        ...
                        if (audioError === "OverconstrainedError") return "마이크 설정 요구사항을 만족할 수 없습니다.";
                    ...
                    <div className="ux_audio_permission_notice_body">
                      {(() => {
                        if (audioError === "NotAllowedError") {
                          if (micPermissionUiState === "blocked") {
                            return (
                              <>
                        ...
                        if (
                          audioError === "NotFoundError" ||
                          audioError === "NotReadableError" ||
                          audioError === "OverconstrainedError"
                        ) {
                          return (
                            <>
                    ...
                    {audioError === "NotAllowedError" && micPermissionUiState === "blocked" && (
