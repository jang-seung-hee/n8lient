# N8Lient n8n 워크플로우 마이그레이션 지시 프롬프트 v2.4

> v2.4 변경 요지: 워크플로우별 입력 Validation 계약 추출을 강화한다. `titleRequired`, `acceptedInputTypes`, `requiredInputMode`, `requiredInputTypes`, `allowedFileTypes`, `maxFileSizeMB`, `maxFiles`, `configSchema.conditionalRequired` 후보를 마이그레이션 보고에 반드시 포함한다.
...
14. processorResult 생성 상태
15. callback 구조 상태
16. Sticky Note 갱신 필요 여부
17. 노드 시각화 배치/번호 체계 정리 필요 여부
18. Google Drive / Optional Export 구조
   - optionalExportProvider
...
11. processorResult 생성 방식
12. callback 구조
13. Sticky Note 영역 구분 반영 여부
14. 노드 시각화 배치/번호 체계 반영 여부
15. Google Drive / Optional Export 처리
