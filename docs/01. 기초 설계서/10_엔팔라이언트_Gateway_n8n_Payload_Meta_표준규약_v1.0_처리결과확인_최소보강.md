...
- 파일명: `10_엔팔라이언트_Gateway_n8n_Payload_Meta_표준규약_v1.0.md`
- 버전: v1.0
...
```text
result.summary → [02] 처리 결과 확인의 짧은 결과 메시지
result.resultUrl → [02] 처리 결과 확인의 대표 링크
processorResult.summary → [02] 처리 결과 확인의 상세 요약
...
10. Gateway callback이 Firestore submissions를 업데이트했는가?
```
