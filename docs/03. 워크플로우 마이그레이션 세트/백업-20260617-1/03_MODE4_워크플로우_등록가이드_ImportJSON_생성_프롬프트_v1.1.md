...
v1.1은 v2.4의 실행 Validation 계약과 Level 1 `notify_only`의 이메일 MD 첨부 정책을 유지하면서, Webhook multipart 입력 원본 파일(file/image/audio 등)을 이메일에 임시 첨부할 수 있는 정책을 추가로 명확히 하기 위한 버전이다.
