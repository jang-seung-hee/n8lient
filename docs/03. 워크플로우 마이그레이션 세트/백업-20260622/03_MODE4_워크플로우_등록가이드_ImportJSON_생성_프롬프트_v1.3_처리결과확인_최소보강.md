# MODE 4 — 워크플로우 등록 가이드 / Import JSON 생성 프롬프트 v1.3
...
# MODE 4 — N8Lient 워크플로우 마스터 등록 HTML 가이드 & Import JSON 생성 프롬프트 v1.3
...
v1.3은 v1.2의 실행 Validation 계약과 Level 1 `notify_only`의 이메일 MD/입력 원본파일 임시 첨부 정책을 유지하면서, `09_엔팔라이언트_워크플로우_버전관리_운영_계약` 기준을 추가로 반영한 버전이다.
...
`5. configSchema` 섹션의 각 설정 카드에는 사용자가 수정할 수 있는 스키마 값과 함께, n8n JSON에서 감지한 기본값 정보를 반드시 표시한다.
...
detectedDefaultValue, defaultWhenOmitted, removalImpact 같은 HTML 내부 표시용 필드는 Import JSON에 export하지 않는다.
```
...
HTML 표시용 기본값 메타데이터(detectedDefaultValue/defaultWhenOmitted/removalImpact/schemaRecommendation 등)는 Import JSON에 export하지 않음
```
...
27. HTML 표시용 기본값 메타데이터가 Import JSON에 불필요하게 export되지 않는가?
```
