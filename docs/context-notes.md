...
### 1. 고객사 등록/수정 시 관리자 및 가입 요청의 원자적(Batch) 동기화 결정
* **배경**: 오퍼레이터가 고객사 마스터에서 `ownerAdminUid`를 정상 조회/매핑하여 저장하더라도, 실제 해당 사용자의 `users/{uid}` 문서와 `companyJoinRequests` 문서가 승인(`approved`) 상태로 정리되지 않아 사용자가 여전히 "승인 대기 중" 화면에서 막혀 있는 비일관성 문제가 존재했습니다.
* **의사결정**:
  - `operatorService.ts`의 `createClient` 및 `updateClient` 서비스 계층에서 `writeBatch`를 통해 고객사 저장 시 지정된 `ownerAdminUid`가 있는 경우 `users/{ownerAdminUid}`의 `clientId`를 배정하고, `role = "company_admin"`, `approvalStatus = "approved"` 상태로 강제 갱신되도록 연동했습니다.
  - 동시에 해당 사용자가 이미 동일 회사로 보낸 pending 상태의 가입 신청서(`companyJoinRequests`)가 존재할 경우, 중복 적재를 방지하기 위해 해당 신청서 역시 `status = "approved"`로 변경하고 승인한 운영자 UID와 처리 시각을 함께 기록하도록 로직을 일원화했습니다.
  - 사용자가 이미 다른 회사(`clientId`)의 승인 완료된 멤버일 경우, 의도치 않은 회사 전환이나 승인 정보 꼬임을 막기 위해 관리자 지정을 사전에 차단하고 예외 메시지를 띄우는 가드레일을 구축했습니다.

### 2. 다원적 오디오 파일(MIME 및 확장자 독립) 검증 정책 수립
* **배경**: 브라우저와 OS에 따라 오디오 파일(예: `.mp3`)을 첨부할 때 검출되는 MIME 타입이 상이하여(예: `audio/mpeg`, `audio/mp3` 등) 기존의 단순 문자열 비교만으로는 정상 파일이 차단되는 사용성 이슈가 발생했습니다.
* **의사결정**:
