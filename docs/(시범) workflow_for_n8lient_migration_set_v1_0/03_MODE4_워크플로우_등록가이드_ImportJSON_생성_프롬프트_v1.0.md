# MODE 4 — 워크플로우 등록 가이드 / Import JSON 생성 프롬프트 v1.0
...
# MODE 4 — N8Lient 워크플로우 마스터 등록 HTML 가이드 & Import JSON 생성 프롬프트 v1.0
...
v1.0는 v2.4의 실행 Validation 계약을 유지하면서, Level 1 `notify_only`의 이메일 MD 첨부 정책을 명확히 하기 위한 버전이다.
...
9. notify_only의 MD 첨부는 resultRefs 또는 Storage 보관으로 판단하지 않는다.
```
...
```text
이메일 알림/보고 기능 또는 이메일 MD 첨부 전송 기능이 있으면 supportsEmailNotification = true
명확하지 않으면 false
...
- MD 첨부파일은 이메일 전송용 임시 결과물이므로 resultRefs/Storage 보관으로 보지 않는다.
...
19. notify_only의 MD 첨부를 supportsResultRefs 또는 Storage 보관으로 오판하지 않았는가?
20. A4 출력 시 버튼이 숨겨지는가?
```
