...
```json
{
  ...
  "error": {
    ...
    "message": "구글 드라이브 ID(googleDriveId) 설정 정보가 유효하지 않아 전송에 실패했습니다."
  }
}
```
...
*   [ ] **에러 콜백 처리**: settings 매핑 대상인 최종 설정값 필드가 존재하지 않거나 유효하지 않으면, 작업을 진행하지 않고 `config_error` 상태 또는 실패 상세 코드를 담아 즉시 `callbackUrl`로 콜백을 반환합니다.
