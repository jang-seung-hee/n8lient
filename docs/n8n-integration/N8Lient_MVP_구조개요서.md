...
5.  **운영자 자동화 템플릿 등록**: 운영자가 `workflowTemplates`에 n8n 연동 규약을 정의한 자동화 템플릿을 등록합니다. (예: `expense-report`, `idea-catcher`)
6.  **운영자 회사별 자동화 배정**: 운영자가 특정 회사(`clientId`)와 템플릿(`workflowKey`)을 묶어 `clientContracts`를 생성합니다.
7.  **회사 관리자 자동화 환경설정**: 회사 관리자는 계약된 자동화 목록을 확인하고, 회사에 필요한 공용 기본 설정값(구글 드라이브 ID, 대표 이메일 등)을 입력하여 `clientAutomations`를 생성 및 `configStatus`를 `configured`로 변경합니다. **이 공용 설정은 회사 전체의 기본값(Fallback) 역할을 하며, 개별 사용자 설정이 존재하는 경우 사용자 개인 설정이 우선 적용됩니다.**
...
*   **운영자 영역 (`/operator`)**: 통합 관리를 위한 별도 운영 콘솔
    ...
    *   `/operator/workflow-templates` (자동화 템플릿 신규 등록, 수정, 복제)
...
*   **운영자 콘솔**: 템플릿 등록/수정/복제 인터페이스 구축 (수정 시 `workflowKey` 및 스키마 키 잠금 기능 적용). 회사 계약 배정 처리 완료.
*   **회사 관리자 설정**: 계약된 템플릿의 `configSchema`를 읽어 동적 설정 폼 빌드 및 `clientAutomations.settings` 저장 기능 구현 완료.
...
n8n 워크플로우를 수정하거나 새로 연동할 때 아래 보안 규칙을 위반해서는 안 됩니다.
