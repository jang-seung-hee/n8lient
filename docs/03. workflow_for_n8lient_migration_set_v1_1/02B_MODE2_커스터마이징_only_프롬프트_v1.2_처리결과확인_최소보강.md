# MODE 2 — N8Lient 커스터마이징 Only 프롬프트 v1.2

이 문서는 일반 n8n 워크플로우를 N8Lient 표준으로만 마이그레이션하는 모드다.
이메일 디자인을 새로 적용하지 않는다.

## 모드 2의 목적

- Gateway / inputSchema / configSchema / retentionPolicy / processorResult / callback 구조를 N8Lient 표준에 맞춘다.
- 이메일 노드가 있더라도 기존 이메일 디자인은 유지한다.
- 단, N8Lient 표준 정책상 필요한 이메일 발송 조건, notify_only MD 임시 첨부, 입력 원본파일 이메일 임시 첨부, settings 키 정리 등은 변경 대상 노드로 보고 후 승인받는다.

## 금지

```text
- 이메일 프레임 템플릿을 새로 적용하지 않는다.
- 본문 HTML 디자인을 새 규칙으로 재작성하지 않는다.
- 단순 미관 개선을 이유로 업무 처리 로직을 수정하지 않는다.
```
...
# N8Lient n8n 워크플로우 마이그레이션 지시 프롬프트 v2.7

> v2.7 변경 요지: v2.6의 입력 Validation 계약과 Level 1 `notify_only`의 이메일 MD/입력 원본파일 임시 첨부 정책을 유지하면서, `09_엔팔라이언트_워크플로우_버전관리_운영_계약` 준수 규칙을 추가한다. n8n JSON을 수정하거나 신규 산출물을 생성할 때 변경 유형(PATCH/MINOR/MAJOR), 권장 workflowVersion, 기존 고객 적용 가능 여부, migrationRequired 여부, rollbackTarget을 함께 보고한다.
...
## 0-1. 워크플로우 버전관리 운영 계약

워크플로우 JSON을 수정하거나 신규 산출물을 생성할 때는 `09_엔팔라이언트_워크플로우_버전관리_운영_계약`을 따른다.
...
- 권장 다음 workflowVersion
- backwardCompatible 여부
- migrationRequired 여부
- 기존 고객 자동 적용 가능 여부
- 선택 업그레이드 필요 여부
- rollbackTarget
- n8n 워크플로우명 버전 표기 필요 여부
- webhookPath 또는 n8nWorkflowId 확인 필요 여부
```

PATCH라도 기존 고객에게 무조건 자동 적용하지 않는다. 테스트 후 운영자 승인으로 clientAutomation.workflowVersion을 전환하는 것을 원칙으로 한다.
...
최종 보고에는 아래 항목을 별도로 포함한다.

```text
- 최종 workflowVersion 후보
...
- n8nWorkflowName 권장값
- webhookPath 또는 n8nWorkflowId 확인 필요 여부
- 기존 고객 적용 방식: PATCH 전환 / 선택 업그레이드 / 별도 마이그레이션
```
...
n8n 워크플로우 커스터마이징 또는 마이그레이션 시 아래 항목을 반드시 검토한다.

```text
1. 이 워크플로우가 완료 후 사용자에게 보여줘야 할 완료 메시지가 있는가?
2. 사용자가 클릭해야 하는 대표 결과 링크가 있는가?
3. 생성 문서, Google Calendar 일정, Google Drive 파일, 외부 결과 페이지 등 후속 액션이 있는가?
4. 해당 정보를 result.summary, result.resultUrl, processorResult.summary, structuredData.actionLinks 중 어디에 넣을 것인가?
5. 이 문구와 링크 라벨은 사용자 의도에 맞게 승인되었는가?
```

### 2. AI 초안 작성 및 사용자 승인 원칙

커스터마이징 AI는 워크플로우 구조와 업무 목적을 분석해 **처리 결과 확인 메시지 초안**을 먼저 작성한다.  
단, 사용자 승인 전에는 해당 문구와 링크 라벨을 최종 반영하지 않는다.
...
### 3. 표시할 주요 링크
| label | url source | callback field |
|---|---|---|

### 4. 권장 callback payload
-

### 5. 사용자 확인 필요사항
- 이 문구로 확정할까요?
- 링크 버튼 label은 적절한가요?
- resultUrl과 actionLinks 중 어떤 표시 방식을 우선할까요?
```

사용자가 승인한 뒤에만 callback payload 생성 노드에 반영한다.

### 3. 권장 노드 구조

callback 직전 사용자 표시용 메시지를 만드는 노드를 둔다.
...
`[24A] Build User Result Message`는 앱의 `[02] 처리 결과 확인` 섹션에 표시될 사용자용 결과 메시지와 링크를 만든다.
...
```json
{
  ...
  "processorResult": {
    "title": "결과 제목",
    "summary": "처리 결과 확인 섹션에 표시할 요약",
    "content": "상세 결과 본문",
    "mdContent": "마크다운 상세 결과",
    "structuredData": {
      "actionLinks": [
        {
...
result.summary
- 짧은 완료 메시지
- notify_only에서도 최소 로그 요약으로 사용할 수 있음

result.resultUrl
- 대표 결과 URL 1개
- 예: 구글 캘린더 일정 링크, 생성 문서 링크, 외부 결과 확인 링크

processorResult.summary
- [02] 처리 결과 확인 섹션에 표시할 사용자용 요약
- processed_result 이상에서 권장

processorResult.structuredData.actionLinks
- 결과 상세 화면에 표시할 액션 링크 목록
- 여러 개의 후속 링크가 있을 때 사용
```

### 5. 예시: 구글 캘린더 등록 워크플로우
...
처리 결과 확인 메시지와 링크에는 아래 값을 포함하지 않는다.
...
Google Calendar event link처럼 사용자의 브라우저 권한으로 열리는 URL은 허용할 수 있다. 단, URL query에 access_token, key, secret 등 민감값이 포함되면 안 된다.
