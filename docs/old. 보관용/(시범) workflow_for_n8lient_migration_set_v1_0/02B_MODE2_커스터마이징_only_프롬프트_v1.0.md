# MODE 2 — N8Lient 커스터마이징 Only 프롬프트 v1.0
...
- 단, N8Lient 표준 정책상 필요한 이메일 발송 조건, notify_only MD 임시 첨부, settings 키 정리 등은 변경 대상 노드로 보고 후 승인받는다.
...
# N8Lient n8n 워크플로우 마이그레이션 지시 프롬프트 v2.5

> v2.5 변경 요지: v2.4의 입력 Validation 계약을 유지하면서, Level 1 `notify_only`의 이메일 MD 첨부 정책을 명확히 한다. `notify_only`에서도 결과 Markdown 파일을 임시 생성해 이메일에 첨부할 수 있으나, N8Lient DB/Storage/resultRefs에는 보관하지 않는다.
...
Level 1 `notify_only`는 이메일/알림 중심 경량형이다. 결과 본문이나 결과 파일을 N8Lient DB/Storage에 보관하지 않는다. 다만 이메일 전송 과정에서 Markdown 결과 파일을 임시 생성하여 본문 또는 첨부파일로 제공할 수 있다.
Level 2 `processed_result` 이상부터는 나중에 검색, 필터링, 지식 인덱싱이 가능하도록 최소 공통 DB 포맷을 맞춘다.

핵심 원칙:
```text
Level 1 notify_only = 이메일 중심 경량형. N8Lient에는 생성 지식 본문/파일을 보관하지 않음. 이메일 MD 첨부 가능.
...
`notify_only`의 MD 첨부파일은 이메일 전송용 임시 결과물이다.
따라서 `notify_only` callback에는 processorResult 전체, Storage 경로, resultRefs를 넣지 않는 것을 기본으로 한다.
...
notify_only의 MD 첨부파일은 임시 결과물이며 N8Lient DB/Storage/resultRefs에 보관하지 않는다.
...
`notify_only`에서 생성하는 MD 첨부파일은 이메일 발송용 임시 파일이다.  
이 파일을 Firebase Storage, Google Drive 기본 보관 위치, resultRefs에 저장하면 `notify_only`가 아니다.  
