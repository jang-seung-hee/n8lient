...
```json
{
  ...
  "defaultDriveId": "shared_drive_id_or_empty_for_my_drive",
  ...
  "configSchema": [
    {
      ...
      "type": "text",
      "required": false,
      "description": "My Drive 사용 시 비워둔다. Shared Drive 사용 시 Shared Drive ID를 입력한다."
    },
    {
      "key": "mdFolderId",
      "label": "마크다운 저장 폴더 ID",
...
```json
{
  ...
  "settings": {
    "driveId": "company_shared_drive_id_or_empty_for_my_drive",
...
```json
{
  ...
  "workflowKey": "idea-catcher",
  "settings": {
    "driveId": "user_shared_drive_id_or_empty_for_my_drive",
...
`driveId`도 같은 동치 규칙을 따른다. 다만 `driveId`는 선택값이며, My Drive 사용 시 생략하거나 빈 문자열로 둘 수 있다. Shared Drive 사용 시에는 `driveId`에 Shared Drive ID를 넣고, `mdFolderId`와 `originalFileFolderId`에는 해당 Drive 내부 폴더 ID를 넣는다.
...
2. **공용 Google Credential 및 Drive 저장소 정책**
   ...
   * 대상 Drive/Sheet/Folder는 해당 Credential 계정 또는 서비스 계정 이메일에 쓰기 권한으로 공유되어 있어야 한다.
...
## 9. Drive 저장소 및 Service Account 호환 정책

### 9.1 `driveId`의 의미

`driveId`는 Google Drive 저장소 단위를 식별하는 선택 설정값이다.

| 사용 방식 | `driveId` | 폴더 ID |
| :--- | :--- | :--- |
| My Drive | 비움 또는 `My Drive` 기본값 | My Drive 내부 폴더 ID |
| Shared Drive | Shared Drive ID | Shared Drive 내부 폴더 ID |

`driveId`는 Credential이나 Secret이 아니다. settings에 저장 가능한 대상 리소스 값이다.

### 9.2 Credential 유형별 권장 저장소

| Credential 유형 | 권장 저장소 | 운영 기준 |
| :--- | :--- | :--- |
| Google OAuth 계정 Credential | My Drive 또는 Shared Drive | 해당 OAuth 계정에 대상 폴더 권한 필요 |
| Google Service Account Credential | Shared Drive | 서비스 계정 이메일에 Shared Drive 또는 폴더 편집 권한 필요 |
| Gmail OAuth Credential | Gmail 발송 | Drive 저장 Credential과 분리 가능 |

### 9.3 회사 공용 설정과 개인 설정

회사 공용 자료는 `clientAutomations.settings`에 회사 Shared Drive의 `driveId`, `mdFolderId`, `originalFileFolderId`를 등록하는 것을 권장한다.

개인별 비공개 자료는 가능하면 동일 Shared Drive 안의 개인 제한 폴더를 사용하고, `userAutomationSettings.settings`에 개인 폴더 ID를 등록한다. 개인 My Drive 폴더 사용은 OAuth 계정 기반 예외 구조로 보고 별도 검증 후 허용한다.

### 9.4 권한 오류 처리

아래 경우 n8n 또는 Gateway는 오류를 무시하지 않고 `failed` 또는 `config_error` 후보로 처리한다.

* `driveId`가 잘못된 경우
* `folderId`가 잘못된 경우
* Service Account가 Shared Drive 또는 폴더에 접근 권한이 없는 경우
* OAuth 계정이 대상 My Drive/Shared Drive 폴더에 접근 권한이 없는 경우
* Sheet/Drive 쓰기 권한이 없는 경우
