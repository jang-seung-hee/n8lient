 ...
 */
function validateExecution(params) {
    const issues = [];
    const missingFields = [];
    ...
    if (files.length > 0 && allowedFileTypes.length > 0) {
        ...
        files.forEach((file, index) => {
            ...
            const isMimeAllowed = normalizedAllowed.some(pattern => {
                if (pattern === "audio" && mimeType.startsWith("audio/"))
                    return true;
                if (pattern === "image" && mimeType.startsWith("image/"))
                    return true;
                if (pattern === "video" && mimeType.startsWith("video/"))
                    return true;
