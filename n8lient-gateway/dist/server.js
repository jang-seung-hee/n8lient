 ...
 */
function buildCompletionNotice(finalSettings, templateDoc, retentionPolicy) {
    // 1. 이메일 주소 탐색
    let email = null;
    ...
    if (!email && templateDoc.configSchema) {
        const emailFieldByType = templateDoc.configSchema.find((f) => f.type === "email");
        if (emailFieldByType) {
    ...
    if (!email && templateDoc.configSchema) {
        const emailFieldByLabel = templateDoc.configSchema.find((f) => typeof f.label === "string" && f.label.includes("이메일") && !emailKeys.includes(f.key));
    ...
    if (process.env.NODE_ENV === "development") {
        console.debug("[buildCompletionNotice-email-diagnosis]", {
            ...
            configSchemaFields: (templateDoc.configSchema || []).map((f) => ({ key: f.key, type: f.type, label: f.label, valueInSettings: finalSettings[f.key] })),
            resolvedEmail: email,
        });
    }
    ...
    const isGoogleDriveEnabled = retentionPolicy?.optionalExportProvider === "google_drive" ||
...
app.post("/api/automation/execute", auth_1.checkAuth, upload.single("file_0"), async (req, res) => {
    ...
    try {
        ...
        const coPolicy = autoDoc.companyRetentionPolicy || {
            recommendedLevel: autoDoc.companyRetentionPolicy?.defaultLevel ||
                contractDoc?.contractRetentionLimit?.maxLevel ||
                operatorContractFallback.maxLevel,
            defaultLevel: autoDoc.companyRetentionPolicy?.defaultLevel ||
                contractDoc?.contractRetentionLimit?.maxLevel ||
                operatorContractFallback.maxLevel,
            allowedUserLevels: autoDoc.companyRetentionPolicy?.allowedUserLevels ||
        ...
        const levelResolution = (0, resolveEffectiveRetentionLevel_1.resolveEffectiveRetentionLevel)({
            capabilities,
        ...
        const effectiveOptionalExport = (0, resolveRetentionPolicy_1.resolveEffectiveOptionalExportProvider)(finalLevel, finalSettings.optionalExportProvider);
        Object.assign(finalSettings, (0, resolveEffectiveRetentionLevel_1.applyEffectiveRetentionToSettings)(finalSettings, finalLevel, effectiveOptionalExport));
