...
function getAdminApp() {
    ...
    if (!storageBucket) {
        ...
        console.warn(`[firebaseAdmin] FIREBASE_STORAGE_BUCKET 환경변수가 정의되어 있지 않아 1차 Fallback 버킷('${storageBucket}')을 강제 지정하여 초기화합니다. 정상 업로드 실패 시 env.yaml에 적절한 버킷명을 설정하십시오.`);
    }
...
function getAdminStorage() {
    const app = getAdminApp();
    return firebase_admin_1.default.storage(app);
}
